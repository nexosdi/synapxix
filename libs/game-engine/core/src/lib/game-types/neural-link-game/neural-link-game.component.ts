        ...
        @for (card of cards(); track $index) {
          ...
          >
            ...
            <div class="absolute inset-0 flex flex-col items-center justify-center gap-2 rounded-[2.5rem] border-b-8 bg-white [backface-visibility:hidden] [transform:rotateY(180deg)]"
              ...
              @if (card.imageUrl) {
                <img [src]="card.imageUrl" class="h-24 w-24 object-cover rounded-2xl shadow-sm mb-1" />
              }
              @if (card.text) {
...
export class NeuralLinkGameComponent implements OnInit {
  ...
  flipCard(card: MemoryCard) {
    if (card.isMatched || card.isFlipped || this.flippedCards().length === 2 || this.feedbackState() !== 'idle') return;
