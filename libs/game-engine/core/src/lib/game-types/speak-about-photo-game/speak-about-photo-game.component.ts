...
@Component({
  ...
  template: `
    ...
    >
      ...
      @if (view.imageUrl) {
      <figure
        class="overflow-hidden rounded-[1.5rem] border border-white/70 shadow-xl shadow-sky-900/20"
      >
        <img
          [src]="view.imageUrl"
      ...
      <div class="space-y-3">
        ...
        <ul class="grid gap-2 md:grid-cols-2">
          ...
          >
            {{ keyword }}
            <span
              class="rounded-full bg-emerald-100 px-3 py-1 text-xs font-semibold uppercase tracking-widest text-emerald-600"
            >
              Key
            </span>
          </li>
      ...
      <footer
        class="flex items-center justify-between rounded-2xl border border-slate-200 bg-white/90 px-5 py-4 text-sm text-slate-600 shadow-inner shadow-slate-900/5"
      >
